...
Tests every endpoint in detail with thorough response validation.
Usage:
...
def validate_field(data, field_name, expected_type=None, required=True, description=""):
    ...
    if expected_type and value is not None:
        if not isinstance(value, expected_type):
            print_warning(f"Field {field_name} has wrong type: {type(value).__name__} (expected {expected_type.__name__})")
            return False
    return True
    ...
    if success and status == 200:
        if validate_field(data, "times", dict):
            ...
            if 'times' in data:
                ...
                print(f"    {Colors.BLUE}▸{Colors.ENDC} Asr: {times.get('asr', 'N/A')}")
                print(f"    {Colors.BLUE}▸{Colors.ENDC} Maghrib: {times.get('maghrib', 'N/A')}")
                print(f"    {Colors.BLUE}▸{Colors.ENDC} Isha: {times.get('isha', 'N/A')}")
            tests_passed += 1
        else:
    ...
    if success and status == 200:
        if validate_field(data, "duas", list) and validate_field(data, "count", int):
            ...
            if 'duas' in data and len(data['duas']) > 0:
                ...
                print(f"    {Colors.BLUE}▸{Colors.ENDC} Arabic: {str(first_dua.get('arabic_text', 'N/A'))[:60]}...")
            tests_passed += 1
        else:
    ...
    if success and status == 200:
        if validate_field(data, "distribution", list):
            ...
            if 'distribution' in data and len(data['distribution']) > 0:
                ...
                print(f"    {Colors.BLUE}▸{Colors.ENDC} Total distributed: {total}")
                # Check if amounts are non-zero (fix validation)
                has_amounts = any(d.get('amount', 0) > 0 for d in data['distribution'])
                if has_amounts:
                    print(f"    {Colors.GREEN}✓{Colors.ENDC} Inheritance amounts calculated correctly")
                else:
                    print_warning("All inheritance amounts are zero - fix needed!")
            tests_passed += 1
        else:
...
def test_quran_endpoints():
    ...
    if success and status == 200:
        if validate_field(data, "number", int) and validate_field(data, "ayahs", list):
            ...
            if 'ayahs' in data:
                print(f"    {Colors.BLUE}▸{Colors.ENDC} Ayahs: {len(data['ayahs'])}")
            tests_passed += 1
        else:
    ...
    if success and status == 200:
        if validate_field(data, "surah_number", int) and validate_field(data, "text_uthmani", str):
            ...
            if 'text_uthmani' in data:
                print(f"    {Colors.BLUE}▸{Colors.ENDC} Text: {data['text_uthmani'][:80]}...")
            tests_passed += 1
        else:
    ...
    if success and status == 200:
        if validate_field(data, "verses", list) and validate_field(data, "count", int):
            ...
            if 'count' in data and data['count'] > 0:
                print(f"    {Colors.GREEN}✓{Colors.ENDC} Found {data['count']} verses containing رحمة")
            tests_passed += 1
        else:
...
def test_rag_endpoints():
    ...
    if success and status == 200:
        if validate_field(data, "answer", str) and validate_field(data, "citations", list):
            ...
            if 'metadata' in data:
                llm_used = data['metadata'].get('llm_used', False)
                retrieved = data['metadata'].get('retrieved_count', 0)
                print(f"    {Colors.BLUE}▸{Colors.ENDC} LLM used: {llm_used}")
                print(f"    {Colors.BLUE}▸{Colors.ENDC} Passages retrieved: {retrieved}")
            tests_passed += 1
        else:
    ...
    if success and status == 200:
        if validate_field(data, "collections", dict) and validate_field(data, "total_documents", int):
            ...
            if 'collections' in data:
                for coll, stats in data['collections'].items():
                    count = stats.get('vectors_count', 0)
                    print(f"    {Colors.BLUE}▸{Colors.ENDC} {coll}: {count} documents")
            tests_passed += 1
        else:
