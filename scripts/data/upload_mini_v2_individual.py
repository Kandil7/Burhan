#!/usr/bin/env python3
"""
Upload Mini Dataset V2 files individually to HuggingFace.
"""
...
def main():
    ...
    # Get list of files to upload (excluding stats.json)
    jsonl_files = sorted(DATASET_DIR.glob("*.jsonl"))
    print(f"\nFound {len(jsonl_files)} JSONL files to upload:")
    ...
    for i, f in enumerate(jsonl_files, 1):
        print(f"\n[{i}/{len(jsonl_files)}] Uploading {f.name}...")
        ...
        print(f"  OK {f.name} uploaded successfully!")

    # Also upload stats.json
    stats_file = DATASET_DIR / "stats.json"
    if stats_file.exists():
        print("\nUploading stats.json...")
        with open(stats_file, "rb") as file_obj:
            api.upload_file(
                path_or_fileobj=file_obj,
                path_in_repo="stats.json",
                repo_id=REPO_ID,
                repo_type="dataset",
                commit_message="Upload stats.json",
            )
        print("  OK stats.json uploaded!")
