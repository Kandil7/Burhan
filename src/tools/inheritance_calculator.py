...
class InheritanceCalculator:
    ...
    def _calculate_furood(self, heirs: Heirs) -> list[InheritanceShare]:
        ...
        has_siblings = (heirs.full_brothers + heirs.full_sisters +
                       heirs.paternal_half_brothers + heirs.paternal_half_sisters +
                       heirs.uterine_brothers + heirs.uterine_sisters) >= 2
        
        # Husband's share
        if heirs.husband:
            fraction = (self.HUSBAND_WITH_DESCENDANTS if has_descendants
                       else self.HUSBAND_WITHOUT_DESCENDANTS)
            shares.append(InheritanceShare(
                heir_name="Husband",
                fraction=fraction,
                percentage=0,
                amount=0,
        ...
        if heirs.wife_count > 0:
            fraction = (self.WIFE_WITH_DESCENDANTS if has_descendants
                       else self.WIFE_WITHOUT_DESCENDANTS)
        ...
        if heirs.mother:
            ...
            is_umariyatan = (heirs.father and
                           (heirs.husband or heirs.wife_count > 0) and
                           not has_descendants and
                           not has_siblings)
        ...
        if (heirs.full_sisters > 0 and not has_descendants and
            not heirs.father and heirs.full_brothers == 0):
    ...
    def _identify_asabah(self, heirs: Heirs) -> list[tuple[str, int]]:
        ...
        if (heirs.full_brothers > 0 and not has_descendants and
            not heirs.father and not heirs.paternal_grandfather):
            asabah.append(("Full Brother", heirs.full_brothers))
        
        # Paternal half-brothers (when no full brothers)
        if (heirs.paternal_half_brothers > 0 and not has_descendants and
            not heirs.father and not heirs.paternal_grandfather and
            heirs.full_brothers == 0):
    ...
    def _calculate_asabah_shares(
        self,
        asabah_heirs: list[tuple[str, int]],
        remainder: Fraction,
        estate_value: float
    ...
    def _apply_awl(
        self,
        furood_shares: list[InheritanceShare],
        estate_value: float
    ) -> list[InheritanceShare]:
        """
        ...
        for share in furood_shares:
            ...
            distribution.append(InheritanceShare(
    ...
    def _get_madhhab_note(
        ...
        if madhhab == "hanafi":
            ...
            if heirs.maternal_brothers > 0 and heirs.paternal_half_brothers > 0:
                return ("Hanafi opinion: Maternal siblings may affect paternal "
            ...
            if heirs.paternal_grandfather and (heirs.full_brothers > 0 or
                                              heirs.full_sisters > 0):
                return ("Jumhur opinion: Grandfather is treated like father in "
    ...
    def _generate_notes(
        ...
        notes.append(
            "Funeral expenses and debts should be paid before distribution."
        )
