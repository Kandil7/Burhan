...
class DuaRetrievalTool(BaseTool):
    ...
    def __init__(self, duas_path: Path | None = None, azkar_path: Path | None = None):
    ...
    def _load_duas(self) -> list[dict]:
        ...
        try:
            if self.duas_path.exists():
                with open(self.duas_path, encoding='utf-8') as f:
                    return json.load(f)
    ...
    def _load_azkar(self) -> list[dict]:
        ...
        try:
            if self.azkar_path.exists():
                with open(self.azkar_path, encoding='utf-8') as f:
                    return json.load(f)
        except Exception as e:
