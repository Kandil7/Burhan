...
from src.quran.quotation_validator import QuotationValidatorator
...
class QuranAgent(BaseAgent):
    ...
    async def execute(self, input: AgentInput) -> AgentOutput:
        ...
        try:
            ...
            return AgentOutput(
                answer=f"Error processing Quran query: {str(e)}",
                confidence=0.0,
                metadata={"error": str(e)}
            )
    
    async def _handle_verse_lookup(
        self,
        query: str,
        input: AgentInput
    ) -> AgentOutput:
    ...
    async def _handle_interpretation(
        self,
        query: str,
        input: AgentInput
    ) -> AgentOutput:
    ...
    async def _handle_quotation_validation(
        self,
        query: str,
        input: AgentInput
    ) -> AgentOutput:
        ...
        if validation["is_quran"]:
            ...
            answer = (
                f"❌ **This text is not from the Quran.**\n\n"
                f"{validation.get('suggestion', '')}"
            )
            confidence = 0.0
