"""
Hadith Collection-Aware RAG Agent for Athar Islamic QA system.

This module provides the HadithCollectionAgent with:
- Full RAG pipeline: query_intake → classify_intent → retrieve → rerank → verify → generate → cite
- Arabic text normalization
- Hadith-specific intent classification (HadithTakhrij, HadithSanad, HadithMatn)
- Verification suite integration

Phase 6: Skeleton Agents for Other Domains - Hadith Collection Agent.
"""
...
# Keywords for HadithTakhrij intent (hadith retrieval)
_TAKHRIJ_KEYWORDS = [
    "حديث",
    "تخریج",
    "المحدیث",
...
# Keywords for HadithSanad intent (chain of transmission)
_SANAD_KEYWORDS = [
    "سند",
    "الاسناد",
    "رواة",
...
# Keywords for HadithMatn intent (text analysis)
_MATN_KEYWORDS = [
    "المتن",
    "نص",
    "مضمون",
...
class HadithCollectionAgent(CollectionAgent):
    ...
    Configuration (class-level):
        TOP_K_RETRIEVAL: Number of candidates to retrieve (default: 60)
    ...
    NO_PASSAGES_MESSAGE = "لم أجد أحاديث доста enough to answer your question."

    # System prompt - Arabic
    SYSTEM_PROMPT = """أنت مساعد إسلامي متخصص في علم الحديث النبوي.

التعليمات:
- استند حصراً إلى النصوص الحديثية المُقدَّمة
- استخدم مراجع المصادر [C1]، [C2]، ... بعد كل جملة
-اذكر درجة الحديث إن وُجدت (صحيح، حسن، ضعيف)
-اذكر الراوي والسند إن توفر
-إذا تعارضت الروايات، اعرض الروايات المختلفة
...
المطلوب: أجب على السؤال مستنداً إلى النصوص أعلاه، مع ذكر المصادر."""
    ...
    def __init__(
        ...
        """
        Initialize the Hadith Collection Agent.
        ...
        if config is None:
            strategy = get_strategy_for_agent("hadith_agent")
            verification_suite = build_verification_suite_for("hadith_agent")
    ...
    # -------------------------------------------------------------------------
    # Intent Classification - Hadith-specific
    # -------------------------------------------------------------------------
    ...
    def classify_intent(self, query: str) -> IntentLabel:
        ...
        - HadithTakhrij: for hadith retrieval (حديث, صحيح, etc.)
        - HadithSanad: for chain of transmission (سند, رواة, etc.)
        - HadithMatn: for text analysis (المتن, معنى, etc.)
