"""
Seerah Collection-Aware RAG Agent for Athar Islamic QA system.

This module provides the SeerahCollectionAgent with:
- Full RAG pipeline: query_intake → classify_intent → retrieve → rerank → verify → generate → cite
- Arabic text normalization
- Seerah-specific intent classification (SeerahEvent, SeerahMilad)
- Verification suite integration

Phase 6: Skeleton Agents for Other Domains - Seerah Collection Agent.
"""
...
# =============================================================================
# Seerah Intent Classification Keywords
# =============================================================================

# Keywords for SeerahEvent intent (historical events)
_EVENT_KEYWORDS = [
    "حدث",
    "وقعة",
    "غزوة",
...
# Keywords for SeerahMilad intent (birth and life phases)
_MILAD_KEYWORDS = [
    "ميلاد",
    "ولادة",
    "طفولة",
...
# =============================================================================
# Seerah Collection Agent
# =============================================================================


class SeerahCollectionAgent(CollectionAgent):
    """
    Collection-Aware RAG Agent for Seerah (Prophet Biography).
    ...
    NO_PASSAGES_MESSAGE = "لم أجد نصوص سيرية كافية للإجابة على سؤالك."

    # System prompt - Arabic
    SYSTEM_PROMPT = """أنت مساعد إسلامي متخصص في السيرة النبوية الشريفة.

التعليمات:
- استند حصراً إلى النصوص السيرية المُقدَّمة
- استخدم مراجع المصادر [C1]، [C2]، ... بعد كل جملة
-اذكر الفترة الزمنية أو التاريخ إن توفر
-اذكر المصدر التاريخي (الطبري، ابن هشام، إلخ)
-إذا كانت هناك روايات متعددة، اعرضها وأيها أصح
...
المطلوب: أجب على السؤال مستنداً إلى النصوص أعلاه، مع ذكر المصادر."""
    ...
    def __init__(
        ...
        """
        Initialize the Seerah Collection Agent.
        ...
        if config is None:
            strategy = get_strategy_for_agent("seerah_agent")
            verification_suite = build_verification_suite_for("seerah_agent")
    ...
    # -------------------------------------------------------------------------
    # Intent Classification - Seerah-specific
    # -------------------------------------------------------------------------

    def classify_intent(self, query: str) -> IntentLabel:
        """
        Classify query to Seerah-specific intent.

        Uses keyword matching to determine:
        - SeerahEvent: for historical events (غزوة, معركة, etc.)
        - SeerahMilad: for birth and life phases (ميلاد, طفولة, etc.)
        ...
        """
        # Check for milad keywords first (birth and life phases)
        for keyword in _MILAD_KEYWORDS:
            if keyword in query:
        ...
        # Default to SeerahEvent for general seerah queries
        return IntentLabel.SeerahEvent
    ...
    async def retrieve_candidates(
        ...
        """
        Retrieve candidate passages from Seerah collection.
