"""
Tazkiyah (Spiritual Development) Collection Agent.

Uses YAML config from config/agents/tazkiyah.yaml
"""
...
class TazkiyahCollectionAgent(CollectionAgent):
    """Tazkiyah (Spiritual Development) Collection Agent."""

    name = "tazkiyah"
    COLLECTION = "tazkiyah_passages"
    ...
    NO_PASSAGES_MESSAGE = "لم أجد نصوصاً في التزكية والأخلاق."
    ...
    def _get_system_prompt(self) -> str:
        try:
            with open("prompts/tazkiyah_agent.txt", "r", encoding="utf-8") as f:
                return f.read()
...
النصوص التزكية:
{passages}
قدم الإجابة مع النصائح."""

    def __repr__(self) -> str:
        return f"<TazkiyahCollectionAgent: {self.name}, collection={self.COLLECTION}>"
