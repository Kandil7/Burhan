"""
Islamic History Collection Agent.

Uses YAML config from config/agents/history.yaml
"""
...
class HistoryCollectionAgent(CollectionAgent):
    ...
    NO_PASSAGES_MESSAGE = "لم أجد معلومات تاريخية كافية."
    ...
    def _get_system_prompt(self) -> str:
        try:
            with open("prompts/history_agent.txt", "r", encoding="utf-8") as f:
                return f.read()
        except FileNotFoundError:
            return """أنت متخصص في التاريخ الإسلامي.
استند فقط إلى النصوص التاريخية.
Use مراجع المصادر [C1]، [C2]."""

    def _build_user_prompt(self, query: str, passages: str, language: str) -> str:
        return f"""السؤال التاريخي: {query}
اللغة المطلوبة: {language}
النصوص التاريخية:
{passages}
قدم الإجابة مع الترتيب الزمني."""
