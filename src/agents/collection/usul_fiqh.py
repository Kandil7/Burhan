"""
Usul Fiqh (Principles of Jurisprudence) Collection Agent.

Uses YAML config from config/agents/usul_fiqh.yaml
"""
...
_USUL_IJTIHAD_KEYWORDS = ["اجتهاد", "تقليد", "فتوى", "مستنبط", "حكم"]
_USUL_QIYAS_KEYWORDS = ["قياس", "أصل", "فرع", "دليل"]
...
class UsulFiqhCollectionAgent(CollectionAgent):
    ...
    NO_PASSAGES_MESSAGE = "لم أجد نصوصاً أصولية كافية."
    ...
    async def rerank_candidates(self, query: str, candidates: list[dict]) -> list[dict]:
        threshold = self.strategy.score_threshold if self.strategy else 0.55
    ...
    def _get_system_prompt(self) -> str:
        try:
            with open("prompts/usul_fiqh_agent.txt", "r", encoding="utf-8") as f:
                return f.read()
        except FileNotFoundError:
            return """أنت متخصص في أصول الفقه.
استند حصراً إلى النصوص الأصولية.
Use مراجع المصادر [C1]، [C2]."""

    def _build_user_prompt(self, query: str, passages: str, language: str) -> str:
        return f"""السؤال الأصولي: {query}
اللغة المطلوبة: {language}
النصوص الأصولية:
{passages}
قدم الإجابة الأصولية."""
