"""
UsulFiqh Collection-Aware RAG Agent for Athar Islamic QA system.

This module provides the UsulFiqhCollectionAgent with:
- Full RAG pipeline: query_intake → classify_intent → retrieve → rerank → verify → generate → cite
- Arabic text normalization
- UsulFiqh-specific intent classification (UsulFiqhIjtihad, UsulFiqhQiyas)
- Verification suite integration

Phase 6: Skeleton Agents for Other Domains - UsulFiqh Collection Agent.
"""
...
# =============================================================================
# UsulFiqh Intent Classification Keywords
# =============================================================================

# Keywords for UsulFiqhIjtihad intent (ijtihad methodology)
_IJTIHAD_KEYWORDS = [
    "اجتهاد",
    "الاجتهاد",
    "مستقل",
...
# Keywords for UsulFiqhQiyas intent (analogical reasoning)
_QIYAS_KEYWORDS = [
    "قياس",
    "القياس",
    "الأصل",
...
# =============================================================================
# UsulFiqh Collection Agent
# =============================================================================


class UsulFiqhCollectionAgent(CollectionAgent):
    """
    Collection-Aware RAG Agent for Usul Fiqh (Principles of Jurisprudence).
    ...
    Configuration (class-level):
        ...
        SCORE_THRESHOLD: Minimum score for inclusion (default: 0.55)
        TEMPERATURE: LLM generation temperature (default: 0.15)
        MAX_TOKENS: Maximum tokens in response (default: 2048)
    """

    name = "usul_fiqh_agent"
    ...
    NO_PASSAGES_MESSAGE = "لم أجد نصوص أصولية كافية للإجابة على سؤالك."

    # System prompt - Arabic
    SYSTEM_PROMPT = """أنت مساعد إسلامي متخصص في أصول الفقه الإسلامي.

التعليمات:
- استند حصراً إلى النصوص الأصولية المُقدَّمة
- استخدم مراجع المصادر [C1]، [C2]، ... بعد كل جملة
-اذكر الدليل الأصولي (القرآن، السنة، الإجماع، القياس) إن توفر
-فصّل بين الدليل القطعي والظني
-اذكر opinion من المالكية، الحنفية، الشافعية، الحنابلة إن توفر
...
المطلوب: أجب على السؤال مستنداً إلى النصوص أعلاه، مع ذكر المصادر."""
    ...
    def __init__(
        ...
        """
        Initialize the UsulFiqh Collection Agent.
        ...
        if config is None:
            strategy = get_strategy_for_agent("usul_fiqh_agent")
            verification_suite = build_verification_suite_for("usul_fiqh_agent")
    ...
    # -------------------------------------------------------------------------
    # Intent Classification - UsulFiqh-specific
    # -------------------------------------------------------------------------

    def classify_intent(self, query: str) -> IntentLabel:
        """
        Classify query to UsulFiqh-specific intent.

        Uses keyword matching to determine:
        - UsulFiqhIjtihad: for ijtihad methodology (اجتهاد, استنباط, etc.)
        - UsulFiqhQiyas: for analogical reasoning (قياس, الأصل, etc.)
        ...
        """
        # Check for qiyas keywords first (analogical reasoning)
        for keyword in _QIYAS_KEYWORDS:
            if keyword in query:
                return IntentLabel.UsulFiqhQiyas

        # Check for ijtihad keywords
        for keyword in _IJTIHAD_KEYWORDS:
            if keyword in query:
                return IntentLabel.UsulFiqhIjtihad

        # Default to Ijtihad for general usul_fiqh queries
        return IntentLabel.UsulFiqhIjtihad
    ...
    async def retrieve_candidates(
        ...
        """
        Retrieve candidate passages from UsulFiqh collection.
    ...
    def __repr__(self) -> str:
        return f"<UsulFiqhCollectionAgent: {self.name}, collection={self.COLLECTION}>"
