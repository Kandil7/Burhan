"""
Arabic Language Agent for Athar Islamic QA system.

Answers questions about Arabic language, grammar, literature, poetry, and rhetoric.
Covers: النحو, الصرف, البلاغة, المعاجم, الأدب, الشعر, العروض
"""
...
class ArabicLanguageAgent(BaseAgent):
    """Arabic Language Retrieval Agent."""
    name = "arabic_language_agent"
    COLLECTION = "arabic_language_passages"
    TOP_K_RETRIEVAL = 12
    TOP_K_RERANK = 5
    SCORE_THRESHOLD = 0.65
    TEMPERATURE = 0.15
    MAX_TOKENS = 2048
    SYSTEM = """أنت متخصص في اللغة العربية وعلومها: النحو، الصرف، البلاغة، المعاجم، الأدب، والشعر.
المهم:
- أجب بناءً على النصوص المسترجاعة فقط
- اذكر المراجع [C1]، [C2]
- استخدم الأمثلة من النصوص
- إذا لم توجد نصوص كافية، قل ذلك
- أضف تنبيهاً بالرجوع للمتخصصين"""
    USER = """السؤال: {query}
اللغة: {language}
النصوص المسترجاعة:
{passages}
أجب بناءً على النصوص مع الالتزام بالتعليمات."""
    ...
    async def execute(self, input: AgentInput) -> AgentOutput:
        ...
        try:
            ...
            logger.info("arabic_language_agent.retrieved", query=input.query[:50], count=len(passages))
            ...
            logger.error("arabic_language_agent.error", error=str(e))
