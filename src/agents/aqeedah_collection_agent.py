"""
Aqeedah Collection-Aware RAG Agent for Athar Islamic QA system.

This module provides the AqeedahCollectionAgent with:
- Full RAG pipeline: query_intake → classify_intent → retrieve → rerank → verify → generate → cite
- Arabic text normalization
- Aqeedah-specific intent classification (AqeedahTawhid, AqeedahIman)
- Verification suite integration

Phase 6: Skeleton Agents for Other Domains - Aqeedah Collection Agent.
"""
...
# =============================================================================
# Aqeedah Intent Classification Keywords
# =============================================================================

# Keywords for AqeedahTawhid intent (monotheism)
_TAWHID_KEYWORDS = [
    "توحيد",
    "الله",
    "الرحمن",
...
# Keywords for AqeedahIman intent (faith concepts)
_IMAN_KEYWORDS = [
    "ايمان",
    "الإيمان",
    "بالله",
...
# =============================================================================
# Aqeedah Collection Agent
# =============================================================================


class AqeedahCollectionAgent(CollectionAgent):
    """
    Collection-Aware RAG Agent for Aqeedah (Islamic Theology).
    ...
    NO_PASSAGES_MESSAGE = "لم أجد نصوص عقدية كافية للإجابة على سؤالك."

    # System prompt - Arabic
    SYSTEM_PROMPT = """أنت مساعد إسلامي متخصص في العقيدة الإسلامية.

التعليمات:
- استند حصراً إلى النصوص العقدية المُقدَّمة
- استخدم مراجع المصادر [C1]، [C2]، ... بعد كل جملة
-اذكر الدليل من القرآن أو السنة إن توفر
-تمييز بين ما هو قطعيات وما هو ظنيات
-إذا كانت هناك مذاهب في المسألة، اعرضها
...
المطلوب: أجب على السؤال مستنداً إلى النصوص أعلاه، مع ذكر المصادر."""
    ...
    def __init__(
        ...
        """
        Initialize the Aqeedah Collection Agent.
        ...
        if config is None:
            strategy = get_strategy_for_agent("aqeedah_agent")
            verification_suite = build_verification_suite_for("aqeedah_agent")
    ...
    # -------------------------------------------------------------------------
    # Intent Classification - Aqeedah-specific
    # -------------------------------------------------------------------------

    def classify_intent(self, query: str) -> IntentLabel:
        """
        Classify query to Aqeedah-specific intent.

        Uses keyword matching to determine:
        - AqeedahTawhid: for monotheism (توحيد, أسماء الله, etc.)
        - AqeedahIman: for faith concepts (إيمان, الإيمان بالله, etc.)
        ...
        """
        # Check for tawhid keywords first (higher priority)
        for keyword in _TAWHID_KEYWORDS:
            if keyword in query:
                return IntentLabel.AqeedahTawhid

        # Check for Iman keywords
        for keyword in _IMAN_KEYWORDS:
            if keyword in query:
                return IntentLabel.AqeedahIman

        # Default to Iman for general aqeedah queries
        return IntentLabel.AqeedahIman
    ...
    async def retrieve_candidates(
        ...
        """
        Retrieve candidate passages from Aqeedah collection.
