"""
History Collection-Aware RAG Agent for Athar Islamic QA system.

This module provides the HistoryCollectionAgent with:
- Full RAG pipeline: query_intake → classify_intent → retrieve → rerank → verify → generate → cite
- Arabic text normalization
- History-specific intent classification (IslamicHistoryEvent, IslamicHistoryDynasty)
- Verification suite integration

Phase 6: Skeleton Agents for Other Domains - History Collection Agent.
"""
...
# =============================================================================
# History Intent Classification Keywords
# =============================================================================

# Keywords for IslamicHistoryEvent intent (historical events)
_HISTORY_EVENT_KEYWORDS = [
    "تاريخ",
    "حدث",
    "وقعة",
    "معركة",
...
# Keywords for IslamicHistoryDynasty intent (dynasties and eras)
_DYNASTY_KEYWORDS = [
    "دولة",
    "مملكة",
    "خلافة",
...
# =============================================================================
# History Collection Agent
# =============================================================================


class HistoryCollectionAgent(CollectionAgent):
    """
    Collection-Aware RAG Agent for Islamic History.
    ...
    Configuration (class-level):
        ...
        SCORE_THRESHOLD: Minimum score for inclusion (default: 0.50)
        TEMPERATURE: LLM generation temperature (default: 0.15)
        MAX_TOKENS: Maximum tokens in response (default: 2048)
    """

    name = "history_agent"
    COLLECTION = "islamic_history"
    ...
    NO_PASSAGES_MESSAGE = "لم أجد نصوص تاريخية كافية للإجابة على سؤالك."

    # System prompt - Arabic
    SYSTEM_PROMPT = """أنت مساعد إسلامي متخصص في التاريخ الإسلامي.

التعليمات:
- استند حصراً إلى النصوص التاريخية المُقدَّمة
- استخدم مراجع المصادر [C1]، [C2]، ... بعد كل جملة
-اذكر الفترة الزمنية أو التاريخ الهجري إن توفر
-اذكر المصدر التاريخي (الطبري، ابن الأثير، الكامل، إلخ)
-وضحRelations السياسية والاجتماعية إن وُجدت
-إذا كانت هناك روايات متعددة، اعرضها
...
المطلوب: أجب على السؤال مستنداً إلى النصوص أعلاه، مع ذكر المصادر."""
    ...
    def __init__(
        ...
        """
        Initialize the History Collection Agent.
        ...
        if config is None:
            strategy = get_strategy_for_agent("history_agent")
            verification_suite = build_verification_suite_for("history_agent")
    ...
    # -------------------------------------------------------------------------
    # Intent Classification - History-specific
    # -------------------------------------------------------------------------

    def classify_intent(self, query: str) -> IntentLabel:
        """
        Classify query to History-specific intent.

        Uses keyword matching to determine:
        - IslamicHistoryEvent: for historical events (معركة, فتح, etc.)
        - IslamicHistoryDynasty: for dynasties and eras (دولة, خلافة, etc.)
        ...
        """
        # Check for dynasty keywords first (dynasties and eras)
        for keyword in _DYNASTY_KEYWORDS:
            if keyword in query:
                return IntentLabel.IslamicHistoryDynasty

        # Check for event keywords
        for keyword in _HISTORY_EVENT_KEYWORDS:
            if keyword in query:
                return IntentLabel.IslamicHistoryEvent

        # Default to event for general history queries
        return IntentLabel.IslamicHistoryEvent
    ...
    async def retrieve_candidates(
        ...
        """
        Retrieve candidate passages from History collection.
    ...
    def __repr__(self) -> str:
        return f"<HistoryCollectionAgent: {self.name}, collection={self.COLLECTION}>"
