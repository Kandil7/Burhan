...
class VectorStore:
        ...
        try:
            ...
            for collection_name, config in self.COLLECTIONS.items():
                ...
                if not exists:
                    ...
                    logger.info(
                        "vectorstore.collection_created", collection=collection_name, dimension=config["dimension"]
                    )
    ...
    async def upsert(self, collection: str, documents: list[dict], embeddings: np.ndarray) -> int:
        ...
        try:
            ...
            logger.info("vectorstore.upserted", collection=collection, count=len(points))

            return len(points)

        except Exception as e:
            logger.error("vectorstore.upsert_error", error=str(e))
            raise VectorStoreError(f"Failed to upsert documents: {str(e)}") from e

    async def search(
        self, collection: str, query_embedding: np.ndarray, top_k: int = 10, filters: dict | None = None
    ) -> list[dict]:
        """
        ...
        try:
            ...
            logger.info("vectorstore.search", collection=collection, top_k=top_k, results=len(formatted_results))
    ...
    async def search_with_score_threshold(
        self, collection: str, query_embedding: np.ndarray, top_k: int = 10, score_threshold: float = 0.7
    ) -> list[dict]:
        """
    ...
    def get_collection_stats(self, collection: str) -> dict:
        ...
        try:
            ...
            if hasattr(info, "points_count"):
                ...
                vectors_count = info.get(
                    "points_count", info.get("vectors_count", info.get("indexed_vectors_count", 0))
                )
            else:
                # Try to count points
                try:
