...
class QuranLoader:
    ...
    async def _load_surahs(self) -> list[dict]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            for surah_data in surahs_data:
                ...
                revelation_place = surah_data.get("revelation_place", "meccan").lower()
                # API returns different variants, normalize to database values
                if revelation_place in ["madinah", "medina"]:
                    revelation_place = "medinan"
                elif revelation_place in ["makkah", "mecca"]:
    ...
    async def _load_ayahs_for_surah(
        ...
        while True:
            async with httpx.AsyncClient() as client:
                # FIX: Use chapter number in the URL path, not as query param
                # Correct endpoint: /verses/by_chapter/{chapter_number}
                ayahs_endpoint = f"{QURAN_API_BASE}/verses/by_chapter/{surah_number}"
                ...
                params = {
                    ...
                    "fields": "text_uthmani,text_simple,juz_number,page_number,hizb_number,rub_el_hizb_number,sajdah"
                }

                response = await client.get(ayahs_endpoint, params=params)
                response.raise_for_status()
                data = response.json()
